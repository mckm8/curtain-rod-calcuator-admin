...
export class EndingComponent implements OnInit {
  ...
  onClickDeleteAction(itemId: number) {
    ...
    dialogRef.afterClosed().subscribe(confirmation => {
      if (confirmation) {
        ...
        this.restService.deleteEnding(itemId).subscribe(value => {
          this.refreshTable(this.selectedProductGroup.id, this.selectedColor.id);
        }, error1 => {
  ...
  addEnding() {
    ...
    this.restService.addEnding(ending).subscribe(value => {
      this.restService.setProductGroupForEnding(value['id'], this.selectedProductGroup.id).subscribe(value1 => {
        this.restService.setColorForEnding(value['id'], this.selectedColor.id).subscribe(value2 => {
          this.refreshTable(this.selectedProductGroup.id, this.selectedColor.id)
        })
      });
    }, error1 => {
  ...
  onUploadOutput(output: UploadOutput, itemId: number): void {
    if (output.type === 'allAddedToQueue') { // when all files added in queue
      ...
      const event: UploadInput = {
        type: 'uploadAll',
        url: '/kalkulator-admin/endingDefs/' + itemId + '/image',
        method: 'POST',
        data: {foo: 'bar'}
      };
  ...
  private refreshTable(productGroupId: number, colorId: number) {
    this.loaderService.startLoader("Trwa wczytywanie")
    this.restService.getEndingsByProductGroupIdAndColorId(productGroupId, colorId).subscribe(value => {
  ...
  onNameChange($event: any, id:number) {
    ...
    this.restService.updatenNameForParameter(id, $event.target.value, 'endingDefs').subscribe( value => {
      this.loaderService.stopLoader();
    }, error1 => {
      this.loaderService.stopLoader();
    })
  ...
  onPriceChange($event: any, id:number) {
    ...
    this.restService.updatePriceForParameter(id, $event.target.value, 'endingDefs').subscribe( value => {
      this.loaderService.stopLoader();
    }, error1 => {
      this.loaderService.stopLoader();
    })
