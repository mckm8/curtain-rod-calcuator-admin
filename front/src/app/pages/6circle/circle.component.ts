...
import {Circle, Color, ProductGroup} from "../product";
...
export class CircleComponent implements OnInit {
  ...
  onClickDeleteAction(itemId: number) {
    ...
    dialogRef.afterClosed().subscribe(confirmation => {
      if (confirmation) {
        ...
        this.restService.deleteCircle(itemId).subscribe(value => {
          this.refreshTable(this.selectedProductGroup.id, this.selectedColor.id);
        }, error1 => {
  ...
  addCircle() {
    ...
    circle.name = this.newName;
    circle.price = this.newPrice;
    this.loaderService.startLoader("Proszę czekać")
    this.restService.addCircle(circle).subscribe(value => {
      this.restService.setProductGroupForCircle(value['id'], this.selectedProductGroup.id).subscribe(value1 => {
        this.restService.setColorForCircle(value['id'], this.selectedColor.id).subscribe(value2 => {
          this.refreshTable(this.selectedProductGroup.id, this.selectedColor.id)
        })
      });
    }, error1 => {
  ...
  onUploadOutput(output: UploadOutput, itemId: number): void {
    if (output.type === 'allAddedToQueue') { // when all files added in queue
      ...
      const event: UploadInput = {
        type: 'uploadAll',
        url: '/kalkulator-admin/circleDefs/' + itemId + '/image',
        method: 'POST',
        data: {foo: 'bar'}
      };
  ...
  private refreshTable(productGroupId: number, colorId: number) {
    this.loaderService.startLoader("Trwa wczytywanie")
    this.restService.getCirclesByProductGroupIdAndColorId(productGroupId, colorId).subscribe(value => {
      this.circlesByPgAndColor = value;
      this.loaderService.stopLoader();
    }, error1 => {
      this.loaderService.stopLoader();
    })
  ...
  onNameChange($event: any, id:number) {
    ...
    this.restService.updatenNameForParameter(id, $event.target.value, 'circleDefs').subscribe( value => {
      this.loaderService.stopLoader();
    }, error1 => {
      this.loaderService.stopLoader();
    })
  ...
  onPriceChange($event: any, id:number) {
    ...
    this.restService.updatePriceForParameter(id, $event.target.value, 'circleDefs').subscribe( value => {
      this.loaderService.stopLoader();
    }, error1 => {
      this.loaderService.stopLoader();
    })
